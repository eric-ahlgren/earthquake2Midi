...
def generateMidi(days, minmag, location, tempo, base, octave_range, key, patches, outfile=os.path.join(DATA_DIR,'static','quakes.mid')):
	dates = getDateRange(days)
	getRequest(dates, minmag)
	domains = parseQuakes(location)
	#domains = {'distance': (33.69, 11845.77), 'depth': (0.42, 599.18), 'magnitude': (2.5, 7.2)}
	quake_midi = MIDITime(outfile=outfile, tempo=tempo, base_octave=base, octave_range=octave_range)
	track_list = create_track_list(quake_midi, domains, key, patches)
